...
/**
 * Get time duration string without "ago" (e.g., "2 hours", "8 months")
 * Useful for "Time in Market" type displays
 */
export function getTimeDuration(date: Date | string | number): string {
  const d = new Date(date);
  const now = new Date();
  const seconds = Math.floor((now.getTime() - d.getTime()) / 1000);
